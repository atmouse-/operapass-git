...
class pwTextData():
    def __init__(self):
        ...
        self.fldsinfo=""
        self.flds_count=0
        self.flds=[] # flds is encrypt data [(tag_id,[pwRawData,pwRawData,pwRawData]),]
        self.fields=[] # fields is readable text data [(type,text),]
    ...
    with open(filepath,"rb") as fp:
        ...
        while 1:
            ...
            while 1:
                ...
                if l==0 :continue
                if l>0 and l<8:n=fp.read(16);print("ord(2)",l,"pass",fp.tell());continue
                if l>=255 and l<65535:print("ord(l)",l,"break",fp.tell());break
                if l>=35535 :n=fp.read(4);continue
                #if l<8 and l>3:max_count-=1;continue
                #elif l==1 or l==2 or l==3 or l>=32767:print("ord(l)",l,"break",fp.tell());break
                if l>=8 and l<255 :pd.other.append(getBlockData(fp,l))
            ...
            for i in xrange(pd.flds_count):
                ...
                for j in xrange(3):
                    l=int("%d"%struct.unpack('>I',fp.read(4)))
                    if l<9 :print("ord(l)",l,fp.tell());continue
...
if __name__ == "__main__":
    pwfile=sys.argv[1]
    pwands=getData(pwfile)
    print("run")
    for pwand in pwands:
        DecryptPwTextData(pwand)
        print(pwand.key)
        print(pwand.timestamp)
        print(pwand.onurl)
        #print(pwand.unknow_url)
        #print(pwand.domain)
        #print("other2_len",len(pwand.other))
        if pwand.other:
            print("otherUrlInfo:",pwand.other2)
        
        #print(pwand.fldsinfo_len)
        #print(pwand.fldsinfo)
        print("data_fields_count",pwand.flds_count)
        #print(pwand.flds) # flds is encrypt data [(tag_id,[pwRawData,pwRawData,pwRawData]),]
        for i in pwand.fields:
            print('\t',i) # fields is readable text data [(type,text),]
        print("*"*50)
    print("there has",len(pwands),"areas")
